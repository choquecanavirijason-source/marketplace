...
export class SessionEntity {
  ...
  constructor(props: SessionProps) {
    this.props = {
      ...props,
      ip: props.clientIp ?? props.ip ?? props.ipAddress,
      ipAddress: props.clientIp ?? props.ip ?? props.ipAddress,
      lastSeenAt: props.lastSeenAt ?? new Date(),
      revokedAt: props.revokedAt ?? (props.isRevoked ? new Date() : null),
    };
  }
  ...
  get isExpired(): boolean { return new Date().getTime() >= this.props.expiresAt.getTime(); }

  isValid(): boolean {
    return !this.isRevoked && !this.isExpired;
  }

  rotate(newHash: string) {
