...
export class PostgresUserRepository implements UserRepositoryPort {
  ...
  private async hydrateUser(userRow: typeof usersTable.$inferSelect): Promise<UserEntity> {
    ...
    const addresses: AddressProps[] = addressRows.map((a) => ({
      id: a.id,
      userId: a.userId,
      label: a.label,
      country: a.country,
    ...
    const onboardingStates: OnboardingStateProps[] = onboardingRows.map((o) => ({
      id: o.id,
      userId: o.userId,
      stepCode: o.stepCode,
      status: o.status,
  ...
  async list(
    ...
    const users = await Promise.all(rows.map((r) => this.findById(r.id)));
