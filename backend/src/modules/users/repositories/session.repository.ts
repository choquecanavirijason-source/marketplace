...
import { AuthRepositoryPort, SecurityEventRecord } from '../interfaces/auth-repository.interface';
...
export class SessionRepository implements AuthRepositoryPort {
  ...
  async findActiveSessionsByUserId(userId: string): Promise<SessionEntity[]> {
    return this.findUserSessions(userId);
  }
  ...
  async findValidVerificationToken(
    ...
    return { id: rows[0].id, metadata: rows[0].metadata ?? null };
  }
  ...
  async findValidVerificationTokenByHash(
    ...
    if (rows.length === 0) return null;
    return { id: rows[0].id, userId: rows[0].userId, metadata: rows[0].metadata ?? null };
  }
