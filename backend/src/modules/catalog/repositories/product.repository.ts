...
export class ProductRepository {
  ...
  async ensureUniqueSlug(name: string, excludeId?: number): Promise<string> {
  ...
  private async fetchImages(productIds: number[]): Promise<Map<number, ProductImageRef[]>> {
  ...
  async paginate(filters: ProductFilters): Promise<{ items: ProductRow[]; total: number }> {
  ...
  async create(input: ProductCreateInput & { isActive?: boolean }): Promise<ProductRow> {
  ...
  private async replaceImages(productId: number, images: ProductImageRef[]): Promise<void> {
  ...
  async update(id: number, patch: Partial<ProductCreateInput> & { isActive?: boolean }): Promise<ProductRow | null> {
    const existing = await this.drizzle.db
      .select()
      .from(productsTable)
      .where(and(eq(productsTable.id, id), isNull(productsTable.deletedAt)))
  ...
  async setStatus(id: number, status: string): Promise<ProductRow | null> {
    const existing = await this.drizzle.db
  ...
  async categoryNameOf(id: number): Promise<string | null> {
    const rows = await this.drizzle.db
