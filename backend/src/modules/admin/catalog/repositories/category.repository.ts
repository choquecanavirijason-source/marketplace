...
export class CategoryRepository {
  ...
  ensureUniqueSlug = async (base: string, excludeId?: number): Promise<string> => {
  ...
  findById = async (id: number): Promise<CategoryWithCount | null> => {
    const rows = await this.drizzle.db
      .select()
      .from(categoriesTable)
  ...
  adminList = async (filters: CategoryListFilters): Promise<{ items: CategoryWithCount[]; total: number }> => {
  ...
  create = async (data: NewCategoryDb & { name: string }): Promise<CategoryWithCount> => {
