...
export class OrderRepository {
  ...
  async getStats() {
    ...
    const [productsCountRow] = await this.drizzle.db
      .select({ count: count() })
      .from(productsTable);
    const totalProducts = Number(productsCountRow?.count ?? 0);
    ...
    const now = new Date();
    const ordersByMonth = Array.from({ length: 6 }).map((_, idx) => {
      const d = new Date(now.getFullYear(), now.getMonth() - (5 - idx), 1);
      const mKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
      const matching = allOrders.filter((o) => {
        const od = o.createdAt instanceof Date ? o.createdAt : new Date(o.createdAt);
        return od.getFullYear() === d.getFullYear() && od.getMonth() === d.getMonth() && o.status !== 'cancelado';
      });
      return {
        month: mKey,
        ...
        revenue: matching.reduce((acc, o) => acc + Number(o.total), 0),
      };
    });

    const topItems = await this.drizzle.db
      .select({
        productId: orderItemsTable.productId,
        name: orderItemsTable.productName,
        totalSold: sql<number>`cast(sum(${orderItemsTable.quantity}) as integer)`,
        totalRevenue: sql<number>`cast(sum(${orderItemsTable.subtotal}) as numeric)`,
      })
      .from(orderItemsTable)
    ...
    const topProducts = topItems.map((t) => ({
      ...
      product_id: Number(t.productId),
      total_sold: Number(t.totalSold ?? 0),
      total_revenue: Number(t.totalRevenue ?? 0),
    }));
