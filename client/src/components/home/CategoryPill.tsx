...
export const CategoryPill = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex flex-col items-center gap-1.5 sm:gap-2 p-3 sm:p-4 rounded-2xl border-2 transition-all duration-200 hover:-translate-y-1 hover:shadow-md group active:scale-95 cursor-pointer",
    ...
    >
      ...
      >
        <CategoryIcon
          category={category}
          className="w-4 h-4 sm:w-5 sm:h-5 text-foreground/70 transition-colors group-hover:text-foreground"
          strokeWidth={1.75}
        />
      </span>
      ...
      <span className="text-[9px] sm:text-[10px] text-muted-foreground">
        {category.products_count ?? category.count ?? 0} productos
      </span>
    </button>
  );
};
